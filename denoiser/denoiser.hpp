...
public:
    ...
    Scalar alpha = decltype(alpha)(500.0);
    Scalar r = decltype(r)(5.0);

    template<typename DerivedV>
    Denoiser(const Eigen::MatrixBase<DerivedV> &V, const Eigen::MatrixXi &F) {

        Eigen::MatrixXi EF, EI;
        Eigen::VectorXi EMAP;
        igl::edge_flaps(F, uE, EMAP, EF, EI);
        std::cout << "Denoiser: found " << uE.rows() << " unique edges." << std::endl;

        std::vector<Eigen::Triplet<Scalar, int>> nbl;
        for (int jj = 0; jj < uE.rows(); ++jj) {
            if (EF(jj,0)<0 || EF(jj,1)<0) {
                std::cout << "[ Warning ] Denoiser: found boundary edge." << std::endl;
                continue;
            }
